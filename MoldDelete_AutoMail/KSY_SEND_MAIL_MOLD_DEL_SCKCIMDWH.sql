--==========================================================================================
--  RTS.KSY_SEND_MAIL  (DB : SCKCIMDWH.WORLD  /  Data Modify - Mold.aspx 용)
--
--  [ 추가 위치 ]
--    기존 프로시저 맨 마지막 분기(P_MODE = 'DPELOG_RELEASE') 블록이 끝나고
--    최상위 "    END IF;" (EXCEPTION 바로 위, 원본 3546 라인) 바로 앞에 아래 ELSIF 블록을 붙여넣는다.
--
--        ...
    ...
    THEN                          -- DATA MODIFY 사이트에서 MOLD ELOG 데이터 삭제 시 알림메일
        ...
        BEGIN
            ...
            THEN                                 -- 넘어온 데이터 없으면 보내지 말자
                L_SENDER := 'CIM_MANAGER@jcetglobal.com';
